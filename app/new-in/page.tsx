    ...
    >
      <div
        ...
        style={{
          ...
          animation: isMobile
            ? "niSlideUp 0.3s cubic-bezier(0.34,1.1,0.64,1)"
            : "niPopIn 0.28s cubic-bezier(0.34,1.2,0.64,1)",
          position: "relative",
        }}
      >
        <button
      ...
      <style>{`
        ...
        @keyframes niSlideUp { from{opacity:0;transform:translateY(50px)} to{opacity:1;transform:translateY(0)} }
        @keyframes niPopIn   { from{opacity:0;transform:scale(0.96) translateY(10px)} to{opacity:1;transform:scale(1) translateY(0)} }
      `}</style>
    </div>
  )
}
...
export default function NewInPage() {
  ...
  return (
    <>
      <style>{`
        .ni-section { padding: 100px 150px 80px; background: #fff; }
        .ni-header { text-align: center; margin-bottom: 60px; }
        .ni-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 24px; max-width: 1280px; margin: 0 auto; }
        @media (max-width: 900px) { .ni-section { padding: 80px 66px 60px; } .ni-grid { grid-template-columns: repeat(2, 1fr); gap: 16px; } }
        @media (max-width: 480px) { .ni-section { padding: 70px 42px 50px; } .ni-grid { grid-template-columns: repeat(1, 1fr); gap: 12px; } }
      `}</style>

      <main className="ni-section">
