    ...
    >
      <div
        ...
        style={{
          ...
          animation: isMobile
            ? "tsSlideUp 0.3s cubic-bezier(0.34,1.1,0.64,1)"
            : "tsPopIn 0.28s cubic-bezier(0.34,1.2,0.64,1)",
          position: "relative",
        }}
      >
        {/* Close Button */}
      ...
      <style>{`
        ...
        @keyframes tsSlideUp { from{opacity:0;transform:translateY(50px)} to{opacity:1;transform:translateY(0)} }
        @keyframes tsPopIn   { from{opacity:0;transform:scale(0.96) translateY(10px)} to{opacity:1;transform:scale(1) translateY(0)} }
      `}</style>
    </div>
  )
}
    ...
    >
      ...
      <div style={{
        ...
        <span style={{
          ...
          maxWidth: "62%", lineHeight: 1.3, textShadow: "0 1px 4px rgba(0,0,0,0.5)",
        }}>{product.name}</span>
        <span style={{
          background: "rgba(255,255,255,0.95)", color: "#111",
          fontSize: "0.7rem", fontWeight: 700,
          borderRadius: "20px", padding: "3px 9px", flexShrink: 0,
        }}>PKR {product.price.toLocaleString()}</span>
      </div>
      <div style={{
...
// ✅ THIS is the reusable template all 5 pages use
interface Props {
  category: string
  title: string
  subtitle: string
...
export default function ProductsPageTemplate({ category, title, subtitle }: Props) {
    ...
    <>
      <style>{`
        ...
        @media (max-width: 900px) {
          .ts-section { padding: 80px 66px 60px; }
          .ts-grid { grid-template-columns: repeat(2, 1fr); gap: 16px; }
        }
        @media (max-width: 480px) {
          .ts-section { padding: 70px 42px 50px; }
          .ts-grid { grid-template-columns: repeat(1, 1fr); gap: 12px; }
        }
      `}</style>
